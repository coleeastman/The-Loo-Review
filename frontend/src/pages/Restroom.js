...
const Restroom = () => {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      const restroomResponse = await fetch(`${process.env.BACKEND_URL}/api/restrooms/${id}`);
      ...
      if (restroomResponse.ok) {
        ...
        const reviewResponse = await fetch(`${process.env.BACKEND_URL}/api/reviews/restroom/${restroomName}`);
